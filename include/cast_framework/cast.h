...
/// asynchronous result callback function
/// @param[in] result the result (success/fail)
/// @param[in] imagePort the UDP port that will be used for image streaming (-1 if failed)
/// @param[in] imuPort the UDP port that will be used for IMU streaming (-1 if failed)
/// @param[in] swRevMatch whether the sw revisions between the api and the clarius app match
typedef void (^CusConnectFn)(BOOL result, int imagePort, int imuPort, BOOL swRevMatch);
